        ...
        function loadTaskDataForEdit(taskId) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Set due date
                        if (response.due_date) {
                            $('#editDueDate').val(response.due_date.split(' ')[0]);
                        } else {
                            $('#editDueDate').val('');
                        }
            ...
            // Filter functionality
            $('.filter-btn').click(function() {
                deactivateHorizontalScroll(); // Reset scroll mode when filtering
                $('.filter-btn').removeClass('active');
                $(this).addClass('active');
            ...
            if (kanbanBoard) {
                Sortable.create(kanbanBoard, {
                    ...
                    handle: '.column-header',
                    onStart: function() {
                        deactivateHorizontalScroll(); // Reset scroll mode when dragging
                    },
                    onEnd: function(evt) {
                        var columnOrder = [];
                        $('.column').each(function() {
            ...
            // Delete Task
            $(document).on('click', '.delete-task', function() {
                deactivateHorizontalScroll(); // Reset scroll mode when deleting
                if (confirm('Are you sure you want to delete this task?')) {
                    var taskId = $(this).data('task-id');
