$(document).ready(function () {
    ...
    if (!kanbanBoardContainer) {
        console.error("kanbanBoardContainer element not found");
        return;
    }
    if (!horizontalScrollHint) {
        console.warn(
            "horizontalScrollHint element not found - scroll hint will not work"
        );
    }
    ...
    function initializeModals() {
        try {
            ...
            var addCategoryModalEl =
                document.getElementById("addCategoryModal");
            var taskDetailsModalEl =
            ...
            if (addCategoryModalEl && typeof bootstrap !== "undefined") {
                addCategoryModal = new bootstrap.Modal(addCategoryModalEl);
            }
            if (taskDetailsModalEl && typeof bootstrap !== "undefined") {
    ...
    function updateScrollHint() {
        if (isHorizontalScrollMode && activeColumnId) {
            ...
            horizontalScrollHint.innerHTML =
                '<i class="fas fa-arrows-alt-h"></i><span>Scroll horizontally (Active: Column ' +
                activeColumnId +
                ")</span>";
        } else {
            horizontalScrollHint.style.opacity = "0.5";
            horizontalScrollHint.innerHTML =
                '<i class="fas fa-arrows-alt-h"></i><span>Scroll horizontally</span>';
        }
    }
    ...
    function activateHorizontalScroll(columnId) {
        ...
        $("#column-" + columnId + " .column-content").addClass(
            "no-vertical-scroll"
        );
    }
    ...
    $(document).on(
        "mousedown",
        '.column-header[data-column-scroll="horizontal"]',
        function (e) {
    ...
    $(document).on(
        "mousedown",
        '.column-content[data-column-scroll="vertical"], .task-card, .empty-state',
        function (e) {
    ...
    kanbanBoardContainer.addEventListener("wheel", handleMouseWheel, {
        passive: false,
    });
    ...
    $(document)
        .off("click", ".add-to-column")
        .on("click", ".add-to-column", function (e) {
            e.preventDefault();
    ...
    $(document)
        ...
        .on("click", ".edit-task-btn", function () {
    ...
    $(document)
        .off("click", "#editTaskFromDetailsBtn")
        .on("click", "#editTaskFromDetailsBtn", function () {
            var taskId = $(this).data("task-id");
            if (taskDetailsModal) {
    ...
    $(document)
        .off("click", "#deleteTaskFromDetailsBtn")
        .on("click", "#deleteTaskFromDetailsBtn", function () {
    ...
    function loadTaskDataForEdit(taskId) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#editTaskColumnId").val(
                    response.column_id || "{{ $columns->first()->id ?? 1 }}"
                );
                ...
                // Set CSRF token
                $("#editTaskForm input[name='_token']").val(
    ...
    function closeAllDropdowns() {
        if (
            addTaskSelect2 &&
            addTaskSelect2.select2 &&
            typeof addTaskSelect2.select2 === "function"
        ) {
            try {
        ...
        if (
            editTaskSelect2 &&
            editTaskSelect2.select2 &&
            typeof editTaskSelect2.select2 === "function"
        ) {
            try {
    ...
    function formatCategory(category) {
        ...
        $container.append(
            $("<i></i>").addClass(icon).addClass("me-2").css("color", color)
        );
    ...
    // Function to add new task to DOM
    function addTaskToDOM(taskData) {
        var column = $("#column-" + taskData.column_id);
        if (!column.length) {
            console.error("Column not found:", taskData.column_id);
            return;
        }

        var taskList = column.find(".tasks-list");
        if (!taskList.length) {
            console.error("Task list not found in column:", taskData.column_id);
            return;
        }
        ...
        var taskCardHtml = `
            <div class="task-card ${urgencyClass}" id="task-${taskData.id}"
    ...
    function updateTaskCardInDOM(taskId, taskData) {
        ...
        var currentColumnId = taskCard.closest(".column").data("column-id");
        if (currentColumnId !== taskData.column_id) {
        ...
        // Update classes
        taskCard.removeClass(
            "urgency-none urgency-low urgency-medium urgency-high urgency-overdue"
        );
        ...
        switch (filter) {
            ...
            case "urgent":
                $(".task-card").each(function () {
                    var urgency = $(this).data("urgency");
                    if (
                        !["low", "medium", "high", "overdue"].includes(urgency)
                    ) {
                        $(this).hide();
                    }
                });
    ...
    $(document)
        .off("click", ".read-more-btn")
        .on("click", ".read-more-btn", function () {
            var taskId = $(this).data("task-id");
        ...
        if (taskList) {
            ...
            sortableInstances[columnId] = Sortable.create(taskList, {
                ...
                onEnd: function (evt) {
                    ...
                    var sourceColumnId = $(evt.from)
                        .closest(".column")
                        .data("column-id");
                    var targetColumnId = $(evt.to)
                        .closest(".column")
                        .data("column-id");
                        ...
                        success: function (response) {
                            ...
                            if (targetColumn) {
                                const targetTaskList =
                                    targetColumn.querySelector(".tasks-list");
                                if (targetTaskList) {
                                    // Remove empty-state if task was moved to this column
                                    const emptyState =
                                        targetTaskList.querySelector(
                                            ".empty-state"
                                        );
                                    if (emptyState) {
                                        emptyState.remove();
                                        console.log(
                                            "Removed empty-state from target column"
                                        );
                                    }
                            ...
                            if (sourceColumnId) {
                                const sourceColumn = document.querySelector(
                                    '[data-column-id="' + sourceColumnId + '"]'
                                );
                                if (sourceColumn) {
                                    const sourceTaskList =
                                        sourceColumn.querySelector(
                                            ".tasks-list"
                                        );
                                    if (sourceTaskList) {
                                        const visibleTasks =
                                            sourceTaskList.querySelectorAll(
                                                ".task-card:not(.sortable-ghost)"
                                            ).length;
                                        if (visibleTasks === 0) {
                                            // Add empty-state to source column
                                            const emptyStateHtml =
                                                '<div class="empty-state"><i class="fas fa-tasks text-muted"></i><p class="mb-0">No tasks yet</p></div>';
                                            sourceTaskList.insertAdjacentHTML(
                                                "beforeend",
                                                emptyStateHtml
                                            );
                                            console.log(
                                                "Added empty-state to source column"
                                            );
                                        }
                                    }
                                }
    ...
    // Delete Column
    $(document)
        .off("click", ".delete-column")
        .on("click", ".delete-column", function () {
            if (
                confirm(
                    "Are you sure you want to delete this column? All tasks in this column will be moved to the first column."
                )
            ) {
                var columnId = $(this).data("column-id");
    ...
    // Edit Column
    $(document)
        .off("click", ".edit-column")
        .on("click", ".edit-column", function () {
            deactivateHorizontalScroll(); // Reset scroll mode
            var columnId = $(this).data("column-id");
            ...
            if (
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $(
                            '[data-column-id="' + columnId + '"] .column-title'
                        ).text(newName.trim());
                        // Update data attribute
                        $(
                            '.edit-column[data-column-id="' + columnId + '"]'
                        ).data("column-name", newName.trim());
                        console.log("Column name updated successfully!");
                    },
                    error: function (xhr) {
                        console.log(
                            "Error updating column name. Please try again."
                        );
                    },
                });
            }
    ...
    $(document)
        .off("click", ".color-option")
        .on("click", ".color-option", function () {
    ...
    $(document)
        .off("click", ".icon-option")
        .on("click", ".icon-option", function () {
                    ...
                    try {
                        ...
                        if (jsonResponse.errors) {
                            ...
                            for (var field in jsonResponse.errors) {
                                errorMsg +=
                                    "- " +
                                    jsonResponse.errors[field].join("\n  ") +
                                    "\n";
                            }
